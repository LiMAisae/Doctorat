...
double precision coefav(3  ,nfabor), coefarv(3, nfabor)
...
double precision:: viscap(ncelet)
double precision, dimension(:,:), pointer :: vitenp, vela
double precision, dimension(:), pointer :: imasfl, bmasfl
double precision, dimension(:), pointer :: i_mass_flux_prev, b_mass_flux_prev

double precision, dimension(:), pointer :: brom, broma,crom, croma, crom_prev2
...
iprev  = 1
inc    = 1
print*, "vel, vela",vel(1,50),vela(1,50),"In resopv vel start"
if (ivofmt.lt.0) then
...
do iel = 1, ncel
  ...
  do isou = 1, 3
    trav(isou,iel) = 0.5d0*(vel(isou,iel)*crom(iel) + vela(isou,iel)*croma(iel)) + ardtsr*trav(isou,iel)
  enddo
enddo
...
do ifac = 1, nfabor
  ...
  do isou = 1, 3
    ...
    coefarv(isou, ifac) = 0.5d0*(brom(ifac) + broma(ifac)) * coefav(isou, ifac)
    !do jsou = 1, 3
    !  coefbrv(isou, jsou, ifac) = rho * coefbv(isou, jsou, ifac)
    !enddo
  enddo
enddo
...
!    divu(iel) = divu(iel) + drom*cell_f_vol(iel)/dt(iel)*3.d0/2.d0 + 1.d0/2.d0*divu_prev(iel)
  ...
  enddo
  !print*,'crom, croma, cromaa', crom(50), croma(50), crom_prev2(50),"source term mass aggregation in resopv"
